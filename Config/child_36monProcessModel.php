...
foreach ($observations as $key => $obs) {
  ...
  if ($value != "") {
    ...
    $sql = "SELECT * FROM child_developmet_tests8 WHERE child_id = '$child_id' AND id = '$id'";
    $result = mysqli_query($con, $sql);

    if (mysqli_num_rows($result) > 0) {
      // If the combination already exists, update the observation value
      $sql = "UPDATE child_developmet_tests8 SET observation = '$value' WHERE child_id = '$child_id' AND id = '$id'";
      ...
      $sql = "INSERT INTO child_developmet_tests8 (child_id, id, test, observation) VALUES ('$child_id', '$id', '$test', '$value')";
      if (mysqli_query($con, $sql)) {
        header("Location: ../View/Child/child-developmentView2.php?child_id=$child_id");
      } else {
