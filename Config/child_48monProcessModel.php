...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $observations = array(
    ...
    "observation9" => array("id" => $_POST["id9"], "value" => $_POST["observation9"]),
    "observation10" => array("id" => $_POST["id10"], "value" => $_POST["observation10"]),
    "observation11" => array("id" => $_POST["id11"], "value" => $_POST["observation11"]),
    "observation12" => array("id" => $_POST["id12"], "value" => $_POST["observation12"]),
...
foreach ($observations as $key => $obs) {
  ...
  if ($value != "") {
    ...
    }else if ($key == "observation9") {
      $test = $test9;
      $observation = $observation9;
    }else if ($key == "observation10") {
      $test = $test10;
      $observation = $observation10;
    }else if ($key == "observation11") {
      $test = $test11;
      $observation = $observation11;
    }else if ($key == "observation12") {
      $test = $test12;
    ...
    $sql = "SELECT * FROM child_developmet_tests9 WHERE child_id = '$child_id' AND id = '$id'";
    $result = mysqli_query($con, $sql);

    if (mysqli_num_rows($result) > 0) {
      // If the combination already exists, update the observation value
      $sql = "UPDATE child_developmet_tests9 SET observation = '$value' WHERE child_id = '$child_id' AND id = '$id'";
      ...
      $sql = "INSERT INTO child_developmet_tests9 (child_id, id, test, observation) VALUES ('$child_id', '$id', '$test', '$value')";
      if (mysqli_query($con, $sql)) {
        header("Location: ../View/Child/child-developmentView2.php?child_id=$child_id");
      } else {
