...
if (isset($_POST['update_button'])) {
    if (empty($_POST['updated_test_name']) || empty($_POST['updated_note'])) {
        ...
        if (!empty($_FILES['updated_test_report']['name'])) {
            ...
            $fileSql = "UPDATE doctor_notes SET note_records = '$path' WHERE note_id = '$_POST[note_id]'";
            $fileResult = mysqli_query($con, $fileSql);

            move_uploaded_file($_FILES['updated_test_report']['tmp_name'], $path);
        }
        ...
        $finishUpdate = "UPDATE doctor_notes SET note_topic = '$_POST[updated_test_name]', note_description = '$_POST[updated_note]' WHERE note_id = '$_POST[note_id]'";
                        ...
                        <tbody>
                            ...
                            if ($resultCheck > 0) {
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    <td>' . date("y-m-d") . '</td>
                                    <td><a href="./vog-tests.php?note_id=' . $row['note_id'] . '&mom_id=' . $row['mom_id'] . '" onclick="return confirmUpdate()"><input class="edit-report-btn" name="edite_report" type="button" value="Edit"></a></td>
